...
const filteredBookings = computed(() => {
  ...
  if (selectedDate.value) {
    result = result.filter(b => {
      const bookingDate = new Date(b.registeredAt).toISOString().split('T')[0]
...
const exportData = () => {
  ...
  const data = filteredBookings.value.map(b => ({
    ...
    联系电话: b.userPhone || '-',
    微信号: b.userWechat || '-',
    身份: getRoleText(b.userRole) || '-',
    预约时间: formatDate(b.registeredAt),
    状态: statusText[b.status]
  }))
  ...
  const fileName = `预约数据_${timestamp}.json`
